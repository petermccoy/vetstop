...
/**
 * Plan a trip: origin + destination, find saved locations within a corridor
 * of the route, pick stops, then hand off to Google Maps for navigation.
 */
@Composable
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        Column(
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text(
                    text = "Within %.1f mi of route".format(state.corridorMiles),
                    style = MaterialTheme.typography.bodyMedium,
                    modifier = Modifier.weight(1f),
                )
                Slider(
                    value = state.corridorMiles,
                    onValueChange = viewModel::setCorridorMiles,
                    valueRange = 0.5f..10f,
        ...
        if (route != null) {
            Text(
                text = "${route.distanceText} · ${route.durationText} — " +
                    "${state.candidates.size} location(s) along route, " +
                    "${state.selectedPlaceIds.size} selected " +
                    "(max $MAX_WAYPOINTS stops)",
                style = MaterialTheme.typography.bodyMedium,
                            ...
                            Column(modifier = Modifier.weight(1f)) {
                                ...
                                Text(
                                    text = LocationCategory
                                        ...
                                        " · %.1f mi off route".format(
                                            candidate.distanceFromRouteMeters /
                                                GeoUtils.METERS_PER_MILE
                                        ),
